...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getAccessRights() {
    ...
    const response = await prisma.access_right
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'access_right'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createAccessRight() {
    await accessRightValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.access_right.create({
      data: body,
    });
